...
//   get html element variables
const categoryList = document.getElementById("categoryList");
const activityList = document.getElementById("activityList");
const activityParagraph = document.getElementById("activityParagraph");
...
function showActivityDetails() {
  const activityId = activityList.value;
  for (const activity of activities) {
    if (activity.id == activityId) {
      const details = `You selected the ${activity.name} ${activity.id} located ${activity.location} with a price of $${activity.price}`;
      activityParagraph.innerText = details;
    }
  }
}
loadCategory();
...
activityList.onchange = showActivityDetails;
// wire-up funstions
